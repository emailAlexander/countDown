...
export class TimerView extends LitElement {
    ...
    _updateTime(element: TimerView) {
        element.requestUpdate();
    }

    _isRunning(orPaused: Boolean = true): boolean {
        if (orPaused) {
    ...
    _onStart() {
        this.input = "";
        this.dispatchEvent(new CustomEvent<TimerModel>('start', { detail: this.timer }));
        this._updateTime(this);
    }

    _onStop() {
        this.dispatchEvent(new CustomEvent<TimerModel>('stop', { detail: this.timer }));
        this._updateTime(this);
    }

    _onDelete() {
        this.dispatchEvent(new CustomEvent<TimerModel>('delete', { detail: this.timer }));
    }

    _onBadge() {
        this.dispatchEvent(new CustomEvent<TimerModel>('badge', { detail: this.timer }));
    }

    _onNote(e) {
        this.timer.note = e.target.value;
        this.dispatchEvent(new CustomEvent<TimerModel>('update', { detail: this.timer }));
    }
    ...
    _onSound() {
        this.dispatchEvent(new CustomEvent<TimerModel>('options.sound', { detail: this.timer }));
        this.requestUpdate();
    }

    _onVolume() {
        this.dispatchEvent(new CustomEvent<TimerModel>('options.volume', { detail: this.timer }));
        this.requestUpdate();
    }
    ...
    _onWheelInput(e: WheelEvent) {
        if (this._isRunning()) {
            return;
        }

        var ms = 0;
        if (155 <= e.x && e.x <= 180) ms = 1000;
        if (125 <= e.x && e.x <= 150) ms = 60000;
        if (90 <= e.x && e.x <= 115) ms = 3600000;
        if (45 <= e.x && e.x <= 85) ms = 86400000;
